...
catch { XCTFail("Could not connect: \(error)") }
...
do {
    ...
    guard let result = try connection.query("SELECT * FROM family") where result.rowCount == 1
        else { XCTFail("No results returned") }
...
exit(0)
    ...
    @noreturn
    func XCTFail(string: String) { print(string); exit(1) }

    @noreturn
    func XCTAssert(@autoclosure expression: () -> Bool, _ message: String = "Test failed") {
